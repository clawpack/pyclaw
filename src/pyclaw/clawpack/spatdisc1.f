...
c
c ===================================================================
      subroutine homodisc1(maxmx,num_eqn,num_waves,num_ghost,num_aux,mx, q,aux,dx,dt,
...
c
c     amdq(1-num_ghost:maxmx+num_ghost, num_eqn) = left-going flux-differences
c     apdq(1-num_ghost:maxmx+num_ghost, num_eqn) = right-going flux-differences
c        e.g. amdq(m,i) = m'th component of A^- \Delta q from i'th Riemann
c                         problem (between cells i-1 and i).
c
c     wave(1-num_ghost:maxmx+num_ghost, num_eqn, num_waves) = waves from solution of
c                                           Riemann problems,
c            wave(m,mw,i) = mth component of jump in q across
c                           wave in family mw in Riemann problem between
c                           states i-1 and i.
c
c     s(1-num_ghost:maxmx+num_ghost, num_waves) = wave speeds,
c            s(m,iw) = speed of wave in family mw in Riemann problem between
c                      states i-1 and i.
c
c     f(1-num_ghost:maxmx+num_ghost, num_eqn) = correction fluxes for second order method
c            f(m,i) = mth component of flux at left edge of ith cell 
c     --------------------------------------------------------------------
c
      implicit double precision (a-h,o-z)    
      double precision q(num_eqn,1-num_ghost:maxmx+num_ghost)
      double precision hypdisc(num_eqn,1-num_ghost:maxmx+num_ghost)
      double precision aux(num_aux,1-num_ghost:maxmx+num_ghost)
      double precision f(num_eqn,1-num_ghost:maxmx+num_ghost)
      double precision s(num_waves,1-num_ghost:maxmx+num_ghost)
      double precision wave(num_eqn, num_waves,1-num_ghost:maxmx+num_ghost)
      double precision amdq(num_eqn,1-num_ghost:maxmx+num_ghost)
      double precision apdq(num_eqn,1-num_ghost:maxmx+num_ghost)
      double precision dtdx(1-num_ghost:maxmx+num_ghost)
...
c     -----------------------------------------
c
      call rp1(maxmx,num_eqn,num_waves,num_ghost,mx,q,q,aux,aux,wave,s,amdq,apdq)
c
...
c      # apply limiter to waves:
      if (limit) call limiter(maxmx,num_eqn,num_waves,num_ghost,mx,wave,s,mthlim)
c
      do 120 i=1,mx+1
         do 120 m=1,num_eqn
