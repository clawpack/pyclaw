...
from clawpack.pyclaw.util import check_solutions_are_same, \
    check_ds_sol_is_downsampled_from_a_sol
...
class IOTest():
    ...
    def test_io_from_binary(self):
        ...
        regression_dir = os.path.join(self.test_data_dir,
                                      './advection_2d_binary')
        self.read_write_and_compare(self.file_formats, regression_dir, 'binary',
                                    0)

    def test_io_from_hdf5(self):
        regression_dir = os.path.join(self.test_data_dir,
                                      './Sedov_regression_hdf')
        self.read_write_and_compare(self.file_formats, regression_dir, 'hdf5',
                                    1)

    def test_io_from_hdf5_with_aux(self):
        regression_dir = os.path.join(self.test_data_dir,
                                      './advection_2d_with_aux')
        self.read_write_and_compare(self.file_formats, regression_dir, 'hdf5',
                                    0, aux=True)
    ...
    def test_ds_from_hdf5(self):
        regression_dir = os.path.join(self.test_data_dir,
                                      './Sedov_regression_hdf')
        self.read_downsample_and_compare(self.file_formats, regression_dir,
                                         'hdf5', 1, (2, 2, 2))
    ...
    def read_write_and_compare(self, file_formats, regression_dir,
                               regression_format, frame_num, aux=False):
        ...
        ref_sol.read(frame_num, path=regression_dir,
                     file_format=regression_format, read_aux=aux)
        if aux:
    ...
    def read_downsample_and_compare(self, file_formats, regression_dir,
                                    regression_format, frame_num,
                                    downsampling_factors, aux=False):
        r"""Test downsampling:
        ...
        """
        a_sol = self.solution
        a_sol.read(frame_num, path=regression_dir,
                   file_format=regression_format, read_aux=aux)
        if aux:
        ...
        for fmt in file_formats:
            ...
            a_sol.downsample(aux, False).write(frame_num, path=io_test_dir,
                                               file_format=fmt, write_aux=aux)
