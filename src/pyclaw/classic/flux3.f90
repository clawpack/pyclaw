...
!     # flux differences (in the y-direction).
    ...
    call rpt3(ixyz,2,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d,q1d,aux1, &
    aux2,aux3,1,amdq,bmamdq,bpamdq)

!     # split the right-going flux difference into down-going and up-going
!     # flux differences (in the y-direction).

    call rpt3(ixyz,2,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d,q1d,aux1, &
    aux2,aux3,2,apdq,bmapdq,bpapdq)

!     # split the left-going flux difference into down-going and up-going
!     # flux differences (in the z-direction).

    call rpt3(ixyz,3,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d,q1d,aux1, &
    aux2,aux3,1,amdq,cmamdq,cpamdq)

!     # split the right-going flux difference into down-going and up-going
!     # flux differences (in the y-direction).

    call rpt3(ixyz,3,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d,q1d,aux1, &
    aux2,aux3,2,apdq,cmapdq,cpapdq)
...
!     # in the y-direction and z-direction.
    ...
    if (m3 == 2) then
        ...
        !            # in the y-like direction with imp=1
            call rpt3(ixyz,2,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d, &
            q1d,aux1,aux2,aux3,1,cqxx,bmcqxxm,bpcqxxm)

        !            # in the y-like direction with imp=2
            call rpt3(ixyz,2,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d, &
            q1d,aux1,aux2,aux3,2,cqxx,bmcqxxp,bpcqxxp)

        !            # in the z-like direction with imp=1
            call rpt3(ixyz,3,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d, &
            q1d,aux1,aux2, aux3,1,cqxx,cmcqxxm,cpcqxxm)

        !            # in the z-like direction with imp=2
            call rpt3(ixyz,3,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d, &
            q1d,aux1,aux2,aux3,2,cqxx,cmcqxxp,cpcqxxp)
        else
        ...
        !            # in the y-like direction
            call rpt3(ixyz,2,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d, &
            q1d,aux1,aux2,aux3,0,cqxx,bmcqxxm,bpcqxxm)

        !            # in the z-like direction
            call rpt3(ixyz,3,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d, &
            q1d,aux1,aux2,aux3,0,cqxx,cmcqxxm,cpcqxxm)
...
!     # BCAAu_{xxzy}.
    ...
    if( m4 > 0 )then
        call rptt3(ixyz,2,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d,q1d, &
        aux1,aux2,aux3,2,2,cpapdq2,bmcpapdq,bpcpapdq)
        call rptt3(ixyz,2,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d,q1d, &
        aux1,aux2,aux3,1,2,cpamdq2,bmcpamdq,bpcpamdq)
        call rptt3(ixyz,2,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d,q1d, &
        aux1,aux2,aux3,2,1,cmapdq2,bmcmapdq,bpcmapdq)
        call rptt3(ixyz,2,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d,q1d, &
        aux1,aux2,aux3,1,1,cmamdq2,bmcmamdq,bpcmamdq)
    endif
...
!     # we are re-using the previous storage rather than requiring new arrays.
    ...
    if( m4 > 0 )then
        call rptt3(ixyz,3,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d,q1d, &
        aux1,aux2,aux3,2,2,bpapdq,bmcpapdq,bpcpapdq)
        call rptt3(ixyz,3,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d,q1d, &
        aux1,aux2,aux3,1,2,bpamdq,bmcpamdq,bpcpamdq)
        call rptt3(ixyz,3,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d,q1d, &
        aux1,aux2,aux3,2,1,bmapdq,bmcmapdq,bpcmapdq)
        call rptt3(ixyz,3,maxm,num_eqn,num_waves,num_aux,num_ghost,mx,q1d,q1d, &
        aux1,aux2,aux3,1,1,bmamdq,bmcmamdq,bpcmamdq)
    endif
