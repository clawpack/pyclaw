...
class ImplicitClawSolver1D(ImplicitClawSolver):
    ...
    def updatesolution(self,solutions):
        ...
        nonlinearFun = lambda y : self.nonlinearFunction(qold,y,state)
    ...
    def nonlinearFunction(self,qold,y,state):
        r"""
        ...
        sd,self.cfl = classic1.step1imp(mx,mbc,mx,qapprox,aux,dx,dt,self.method,self.mthlim)

        # Compute the nonlinear vector-valued function
        #assert dq.flags['F_CONTIGUOUS']
        #nlf.setArray(qapprox[:,mbc:-mbc]-dq[:,mbc:-mbc]-qold[o,:])
