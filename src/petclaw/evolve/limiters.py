...
def limit(meqn,wave,s,limiter,dtdx):
    r"""
    ...
    # wave_norm2 is the sum of the squares along the meqn axis,
    # so the norm of the cell i for wave number j is addressed 
    # as wave_norm2[i,j]
    wave_norm2 = np.sum(np.square(wave),axis=1)
    ...
    # along the meqn axis.  For reference, dotls[0,:,:] is the dot
    # product of the 0 cell and the 1 cell.
    dotls = np.sum(wave[1:,:,:]*wave[:-1,:,:],axis=1)
    ...
    for mw in xrange(wave.shape[2]):
        ...
        if limit_func is not None:
            for m in xrange(meqn):
                ...
                wave[1:-1,m,mw] = wave[1:-1,m,mw]*wave_zero_mask[1:-1,mw] \
                    + wlimitr * wave[1:-1,m,mw] * wave_nonzero_mask[1:-1,mw]
...
def van_leer_klein_sharpening_limiter(r,cfl):
    ...
    """
    a = np.ones((len(r),2)) * 1.e-5
    a[:,0] = r
    
    rcorr = np.max(a,axis=1)
    a[:,1] = 1/rcorr
    sharg = np.min(a,axis=1)
...
def theta_limiter(r,cfl,theta=0.95):
    ...
    a[:,0] = 0.001
    a[:,1] = cfl
    cfmod1 = np.max(a,axis=1)
    a[:,0] = 0.999
    cfmod2 = np.min(a,axis=1)
    s1 = 2.0 / cfmod1
    s2 = (1.0 + cfl) / 3.0
    ...
    a[:,0] = (1.0 - theta) * s1
    a[:,1] = 1.0 + s2 * (r - 1.0)
    left = np.max(a,axis=1)
    a[:,0] = (1.0 - theta) * phimax * r
    a[:,1] = theta * s1 * r
    middle = np.max(a,axis=1)
    
    b[:,0] = left
    b[:,1] = middle
    b[:,2] = theta*phimax
    
    return np.min(b,axis=1)
