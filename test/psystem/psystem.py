...
def compute_F(state):
    ...
    dx=state.patch.delta[0]; dy=state.patch.delta[1]
...
def psystem2D(use_petsc=True,solver_type='classic',iplot=False,htmlplot=False):
    ...
    #strain=claw.frames[claw.num_output_times].state.gqVec.getArray().reshape([patch.ng[0],patch.ng[1],state.num_eqn])[:,:,0]
    #return strain
