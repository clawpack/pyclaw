...
c
c     # Modified to add boundary conditions at top and bottom for
c     # quadrilateral grid on the sphere.
c
...
c
  300 continue
c     # boundary conditions for sphere
      do 305 m=1,meqn
         do 305 jbc=1,mbc
            do 305 i = 1-mbc, mx+mbc
               i2 = mx + 1 - i
               
               q(i,1-jbc,m) = q(i2,jbc,m) !????????????????
  305       continue
      go to 399
c
  310 continue
